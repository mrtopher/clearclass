/**
 * U11 — the gated `/api/chat` endpoint.
 *
 * U11 ships the AUTH GATE; U6 fills the body. The ordering is the whole point:
 * the principal + server-derived tenant are resolved BEFORE `runAgent`, so no
 ...
 * never reached on the rejected paths.
 *
 * The entire loop (once U6 wires it) runs server-side only (KTD11); this route
 * file is the single server boundary the client talks to.
 */
 ...
 * backend or a model. `POST` below binds the real dependencies.
 */
